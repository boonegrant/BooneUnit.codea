...
function testBooneUnitFeature() 
    ...
    _:describe( 'Test statements enclosed in a "describe()" statement produce'..
        ...
        thisFeatureInfo = booneUnit:describe( "An Uneventful Group of Tests", function() 
            ...
            testTwoInfo = booneUnit:test( "passable", function() 
                booneUnit:expect( "foo" ).is( "foo" )
            end )
            testThreeInfo = booneUnit:test( "impassable", function() 
    ...
    _:describe( '"FeatureInfo:report()" returns a string summarizing the outcomes of'..
        ...
        local thisFeature = booneUnit:describe( "An Example for Reporting", function() 
            ...
            booneUnit:test( "#3 Passing test", function()
                booneUnit:expect( 3 * 3 ).is( 9 )
            end )
            booneUnit:test( "#1 Failing test", function()
                booneUnit:expect( true ).is( false )
            end )
            booneUnit:test( "#2 Failing test", function()
                booneUnit:expect( "foo" ).is( "bar" )
            end )
            booneUnit:ignore( "#1 Ignored test", function()
                booneUnit:expect( 2 + 2 ).is( 4 )
